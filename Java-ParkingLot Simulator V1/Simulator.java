	...
	public void simulate() {
		...
		while (clock < steps) {
				...
				for (int j = 0; j < lot.getNumSpotsPerRow(); j++) {
					if (lot.getSpotAt(i,j)!= null) {
						...
						if (duration== MAX_PARKING_DURATION) {
							Spot leavingSpot = lot.remove(i,j);
			...
			if (! incomingQueue.isEmpty()) {
				...
				boolean canItPark=lot.attemptParking(dequeuedCar,clock);
				if (canItPark) {
					System.out.println(dequeuedCar + " ENTERED at timestep " + clock + "; occupancy is at " +lot.getTotalOccupancy());
					dequeuedSpot=null;
				}
			}
			if (! outgoingQueue.isEmpty()) {
				Spot dequeued= outgoingQueue.dequeue();
				System.out.println(dequeued.getCar() + " EXITED at timestep " + clock + "; occupancy is at " +lot.getTotalOccupancy());
			}		
